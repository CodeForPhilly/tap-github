            ...
            for r in projects:
                ...
                if schemas.get("project_columns"):
                    for project_column_rec in get_all_project_columns(
                        project_id, schemas["project_columns"], repo_path, state, mdata
                    ):
                        singer.write_record(
                            "project_columns",
                            project_column_rec,
        ...
        with metrics.record_counter("reviews") as reviews_counter:
            for response in authed_get_all_pages(
                ...
                for pr in pull_requests:
                    ...
                    if (
                        bookmark_time
                        and singer.utils.strptime_to_utc(pr.get("updated_at"))
                        < bookmark_time
                    ):
                        return state
                    ...
                    if schemas.get("reviews"):
                        for review_rec in get_reviews_for_pr(
                            pr_num, schemas["reviews"], repo_path, state, mdata
                        ):
                            singer.write_record(
